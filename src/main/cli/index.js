...
const cli = () => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to JsMarkNote and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'jsmarknote-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: jsmarknote [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`JsMarkNote: ${JSMARKNOTE_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'jsmarknote-user-data')
