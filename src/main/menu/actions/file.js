...
import { checkUpdates, userSetting } from './jsmarknote'
...
const handleResponseForSave = async(e, id, filename, pathname, markdown, options, defaultPath) => {
  ...
  // on disk nevertheless but is already tracked by JsMarkNote.
...
ipcMain.on('mt::format-link-click', (e, { data, dirname }) => {
  ...
  if (pathname) {
    // decodeURIComponent() CommonMark #503, allow percent encoded path names to open files. https://github.com/jsmarknote/jsmarknote/issues/57
