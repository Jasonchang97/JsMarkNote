...
export const getLogLevel = () => {
  if (!global.JSMARKNOTE_DEBUG_VERBOSE || typeof global.JSMARKNOTE_DEBUG_VERBOSE !== 'number' ||
    global.JSMARKNOTE_DEBUG_VERBOSE <= 0) {
    return process.env.NODE_ENV === 'development' ? 'debug' : 'info'
  } else if (global.JSMARKNOTE_DEBUG_VERBOSE === 1) {
    return 'verbose'
  } else if (global.JSMARKNOTE_DEBUG_VERBOSE === 2) {
    return 'debug'
  }
  return 'silly' // >= 3
}
